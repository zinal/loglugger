...
- Защита по размеру запроса настраивается через:
  ...
  - Значения по умолчанию: `16777216` (16 MiB) и `33554432` (32 MiB).
- Таймауты HTTP-сервера защищают от Slowloris / зависших соединений (можно переопределить в конфиге):
  - `read_header_timeout` (по умолчанию `10s`): TLS-рукопожатие + заголовки запроса.
  - `read_timeout` (по умолчанию `60s`): весь запрос, включая тело.
  - `write_timeout` (по умолчанию `60s`): обработка запроса и запись ответа.
  - `idle_timeout` (по умолчанию `120s`): время жизни idle keep-alive соединения.
...
- Пакеты клиента дополнительно ограничены 15 MiB несжатого JSON на один запрос (точный размер тела считается инкрементально при сборке батча).
- Если одна запись сама по себе не укладывается в этот лимит вместе с конвертом запроса, клиент завершает работу с ошибкой (запись не отбрасывается молча и не уходит на сервер сверх лимита).
...
Если режим восстановления включен и клиент сталкивается с повреждением журнала, он выводит предупреждение о возможной потере данных, затем пытается заново открыть журнал и продолжить чтение с последней корректной позиции, а при неудаче переходит к попытке чтения начиная с момента сразу после последней корректной временной отметки. При успешном восстановлении уже накопленные неотправенные записи и незавершённое multiline-состояние сохраняются и отправляются с `reset: true` (восстановление продолжается после последнего прочитанного cursor, поэтому очистка этого буфера привела бы к потере данных); затем чтение продолжается. Если восстановление все равно невозможно, клиент завершает работу.
...
- Формат вывода: TSV; в текстовых значениях TAB/CR/LF экранируются как `\t`, `\n` и `\r`, чтобы не ломать структуру TSV.
...
- Перед каждой попыткой выгрузки инструмент удаляет существующие файлы с таким шаблоном имени для `-output-prefix` в `-output-dir`, затем создаёт выходные файлы с `O_EXCL`. Это исключает дубли строк после retry и позволяет повторить запуск с теми же параметрами после ошибки (остальные файлы в каталоге не затрагиваются).
- Transient-сбои YDB во время scan повторяет сам экстрактор (с логированием и очисткой префикса). Автоматический retry SDK вокруг callback записи в файлы отключён, потому что этот callback неидемпотентен.
