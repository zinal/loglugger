...
- `-journal-namespace my_namespace` читает логи из journald namespace, отличного от namespace по умолчанию.
- `-journal-recovery=true` включает режим best-effort восстановления после повреждения журнала (`EBADMSG` / `bad message`). По умолчанию этот режим выключен, так как при обходе поврежденного участка возможна потеря части данных. Без этой опции клиент пишет ошибку о повреждении и сразу завершает работу.
