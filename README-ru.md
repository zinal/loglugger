...
- Защита по размеру запроса настраивается через:
  - `max_compressed_body_bytes`: максимальный размер сырого HTTP body до декодирования `Content-Encoding`.
  - `max_decompressed_body_bytes`: максимальный размер JSON payload после распаковки.
...
- Mapper поддерживает вычисляемые source-поля:
  ...
  - `message_cityhash64`: `CityHash64` от полного payload записи; обычно маппится в `message_hash` (`Uint64`) для уникальности.
- YDB-ориентированные примеры маппинга/схемы: `examples/ydbd/field_mapping.yaml` и `examples/ydbd/target_table.sql`.
...
- Регулярные выражения компилируются и валидируются при старте сервера.
- Некорректные regex теперь приводят к ошибке конфигурации при запуске (а не игнорируются во время TLS handshake).
...
Заметки по схеме/маппингу для YDB:

- В `examples/ydbd/target_table.sql` колонки `log_timestamp_us` и `ts_orig` имеют тип `Timestamp64`.
- В `examples/ydbd/field_mapping.yaml` используйте:
  - `transform: timestamp64_us` для epoch-времени в микросекундах (например, `log_timestamp_us`)
  - `transform: timestamp64` для строковых дат/времени (например, `parsed.P_DTTM` -> `ts_orig`)
- Опция сервера `convert_time_to_local_tz` (по умолчанию `false`) управляет разбором значений `timestamp64` без таймзоны:
  ...
  - `true`: интерпретация в локальной таймзоне ОС перед сохранением (полезно, но опасно при разной timezone-конфигурации хостов)
- Параметры `message_regex` и `message_regex_no_match` теперь задаются на сервере (в YAML/JSON-конфиге сервера).
