...
- Примеры схемы и сопоставления для YDB приведены в `examples/ydbd/field_mapping.yaml` и `examples/ydbd/target_table.sql`.
...
Замечания по схеме и сопоставлению для YDB:
...
- Параметры `message_regex` и `message_regex_no_match` теперь задаются на сервере, в YAML/JSON-конфигурации сервера.
